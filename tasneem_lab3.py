...
# GITHUB REPO LINK - https://github.com/fariha-tasneem/ds2002.git

#With this line of code, we’ve created a new connection object, as well as a new file called orders.db in the directory in which you’re working
conn = sqlite3.connect('orders.db')

#Now that we’ve created a database connection object, our next task is to create a cursor object
cur = conn.cursor()

#Now that we have a connection object (conn) and a cursor object (cur), we can create our first table
...
#Similar to the table generation query, the query to add data uses the cursor object to execute the query.
...
#However, the variable can contain a list, as long as the list items are tuples. For example, we could add more users using the variable:
more_users = [('00003', 'Peter', 'Parker', 'Male'), ('00004', 'Bruce', 'Wayne', 'male')]
cur.executemany("INSERT OR IGNORE INTO users VALUES(?, ?, ?, ?);", more_users)
conn.commit()

#Below are some scripts you can copy and paste to insert some sample data into both tables:
